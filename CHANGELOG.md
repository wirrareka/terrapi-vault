...
- **Phase 0 — CI / supply chain:** `cargo deny` advisories gate fixed (the `rsa` dev-dep ignore
  ...
  duplicate npm lockfile removed. Broker: `VESTA_AUDIT_OS_INSECURE_TLS` dev-gated; insecure-dev +
...
- **Phase 3 — vesta-sync:** rate-limited `/account` + `/enroll`; `deny_unknown_fields` + id length
  caps + per-push op cap; refuses a non-loopback bind without `VESTA_SYNC_DB_KEY`; device list +
...
- **Review follow-up (xhigh code review):** extended fail-closed audit to KMS wrap/unwrap/rotate/
  ...
  legacy users that predate node-tagging. vesta-sync: separate rate bucket for account/enroll vs
...
vesta-console **id_token verification alg fix** — found at the live `acr=mfa` round-trip (the whole
...
Release-CI fix for the vesta-console artifacts — **supersedes 0.1.8**, which built the broker but
...
Carries the **same P1b OIDC RP** as 0.1.8 (below); deploy `vesta-console-0.1.9-{linux,freebsd}`.
...
vesta-console **P1b — OIDC RP login**. The operator console now authenticates humans against
terrapi-identity; this tag is the trigger for the eu console go-live (infra creates the jail / WG
`.110` / dual-EKU cert + edge, identity creates the staged `vesta-console` client, then a live
...
- **OIDC Relying Party** (`vesta-console/src/oidc.rs`) — `authorization_code` + **PKCE (S256)**,
...
- **Cookie session** (`vesta-console/src/session.rs`) — in-memory operator sessions (stateless
...
- `/api/v1/auth/{login,callback,logout}` are now wired (were `501`); the dev stub remains only
  under `VESTA_CONSOLE_ALLOW_INSECURE_DEV`. New env: `VESTA_CONSOLE_OIDC_{ISSUER,REDIRECT_URI,KID}`
  (required to enable login) + optional `CLIENT_ID`/`SCOPES`/`ACR`; the signing key is the existing
  `VESTA_CONSOLE_TLS_KEY`. Unset issuer ⇒ login disabled (dev or all-401).
...
- Config `VESTA_OBJECT_STORE_{KEY,SECRET,REGION,BUCKET}` (+ optional `HOST`/`TTL_SECS`/`MAX_TTL_SECS`);
...
- **OpenSearch clients gain a CA-file TLS option (`opensearch.rs`, `audit_ship.rs`).** New
  `VESTA_OS_CA` / `VESTA_AUDIT_OS_CA` (PEM, mirrors `VESTA_TLS_CLIENT_CA`): when set, the cred-engine
...
- **kms-cap auth = Option J (JWT-bearer).** New `jwt` module verifies identity-minted
  ...
  channel: `alg=ES256`+`kid`, JWKS (OIDC-discovered or `VESTA_KMS_JWT_JWKS_URI`, cached +
  ...
  `VESTA_KMS_JWT_ISSUER` (+ `_AUDIENCE`); absent ⇒ kms stays cap-based (cert-SAN `kms`, the
...
- **arm (a) — identity-sealed master key (unseal client).** New `identity_kms` module: at
  ...
  §2→(A) decision): the broker connects as an mTLS client presenting its own `VESTA_TLS_*`
  ...
  (break-glass) when identity is unreachable. Opt-in via `VESTA_IDENTITY_KMS_URL`; one-time
  bootstrap with `VESTA_KMS_SEAL_INIT=1`. Ships gated off until the dot-form cert is adopted
...
- **Root-rotation handling — arm (a) re-seal + `kms.master_resealed`.** When identity rotates
  ...
  timer (`VESTA_KMS_RESEAL_CHECK_SECS`, default 6 h) for a broker running across a rotation without
...
- **rc.d double user-drop** — the rc.d config vars were named `vault_broker_user` /
  ...
  read the env file (`env: …/vesta-broker.env: Permission denied`), so
  `service vesta-broker start` failed (direct `daemon` worked). Renamed to
  `vault_broker_runas` / `vault_broker_rungroup` (plain vars) so the single `daemon -u`
  drop is used — now reboot-safe via `service`.
- **env + roles perms** — `vesta-broker.env` and `roles.json` are now `0640 root:vault`
...
- **rc.d** — `deploy/rc.d/vesta-broker` no longer inlines a single-quoted `sh -c` in
  `command_args` (rc.subr word-splits it → "Unterminated quoted string"). It runs a new
  `deploy/libexec/vesta-broker-run` wrapper that `set -a`-sources the env drop-in (so
  `VESTA_*` are exported) and execs the broker; the env path is passed via `env(1)`.
...
First release of the **vesta-broker** (Path A), feature-complete for the planned scope.
EU-first deploy on `medina`.

### Broker (services/vesta-broker)
- **Daemon auth** — mutual TLS over WireGuard vs the fleet Root CA; verified peer SAN
  ...
  `session`/`leases`/`kms`/`snapshot`) from `VESTA_ROLES_CONFIG` (deny-all if unset).
