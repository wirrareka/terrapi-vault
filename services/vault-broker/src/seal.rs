...
//! encrypted store (`terrapi_vault::Vault`, SQLCipher) with the operator passphrase. The
...
use std::path::Path;
use terrapi_vault::{Error as VaultError, KdfParams, Vault};
...
mod tests {
    ...
    fn tmp(name: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "vault-broker-seal-test-{name}-{}.sqlcipher",
            std::process::id()
        ))
    }
