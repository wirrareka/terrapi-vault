...
//!
//! Configured from env (`VAULT_OBJECT_STORE_*`); absent → the op is unconfigured (`503`). The
...
impl ObjectStoreSigner {
    /// Build from `VAULT_OBJECT_STORE_*` env, or `None` if `VAULT_OBJECT_STORE_KEY` is unset
    ...
    pub fn from_env() -> Result<Option<Self>, String> {
        let Ok(access_key) = std::env::var("VAULT_OBJECT_STORE_KEY") else {
            return Ok(None);
        };
        let req = |k: &str| {
            std::env::var(k).map_err(|_| format!("VAULT_OBJECT_STORE_KEY set but {k} missing"))
        };
        let secret_key = req("VAULT_OBJECT_STORE_SECRET")?;
        let region = req("VAULT_OBJECT_STORE_REGION")?;
        let bucket = req("VAULT_OBJECT_STORE_BUCKET")?;
        // Virtual-hosted host; overridable for non-DO S3-compatible endpoints / tests.
        let host = std::env::var("VAULT_OBJECT_STORE_HOST")
            .unwrap_or_else(|_| format!("{bucket}.{region}.digitaloceanspaces.com"));
        let default_ttl_secs = env_u64("VAULT_OBJECT_STORE_TTL_SECS", DEFAULT_TTL_SECS);
        let max_ttl_secs = env_u64("VAULT_OBJECT_STORE_MAX_TTL_SECS", DEFAULT_MAX_TTL_SECS);
