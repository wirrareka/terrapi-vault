//! vault-sync metrics: the shared `HttpMetrics` core (requests/latency/inflight + their
//! Prometheus format, in `vault-transport`) plus vault-sync's own domain series (ops
...
impl Metrics {
    ...
    /// Render the full exposition: the shared `vault_sync_http_*` series plus vault-sync's
