...
//! - **vesta-broker** (Svet A — proximi.io platform): per-group, multi-tenant,
//!   residency-air-gapped, mTLS-over-WireGuard.
//! - **vesta-sync** (Svet B — personal apps memento/probe): single-user, E2E,
//!   server-blind, device-keypair auth.
//!
//! NOTHING platform-specific (OpenSearch, RethinkDB, tenants, residency) leaks into
//! `vesta-sync`; this crate only exposes primitives each service opts into. The HTTP
