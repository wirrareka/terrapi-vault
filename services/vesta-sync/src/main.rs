...
//! residency). See `docs/planning/02-vesta-sync-oplog.md`.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Backward-compat for the vault→vesta rename: mirror VAULT_* → VESTA_* before reading config.
    ...
    // warning; set VESTA_SYNC_ALLOW_PLAINTEXT_DB=1 to acknowledge an intentional plaintext run.
    if cfg.db_key.is_none()
        && !cfg.bind.ip().is_loopback()
        && std::env::var("VESTA_SYNC_ALLOW_PLAINTEXT_DB").as_deref() != Ok("1")
    {
        return Err(format!(
            "vesta-sync: refusing a non-loopback bind ({}) without VESTA_SYNC_DB_KEY[_FILE] — the \
             metadata DB would be unencrypted at rest. Set the key, bind 127.0.0.1 for dev, or set \
             VESTA_SYNC_ALLOW_PLAINTEXT_DB=1 to intentionally run plaintext.",
            cfg.bind
        )
        .into());
    }
    let at_rest = if cfg.db_key.is_some() {
        ...
        "PLAINTEXT — set VESTA_SYNC_DB_KEY[_FILE] to encrypt metadata at rest"
    };
    eprintln!(
        "vesta-sync {} starting: bind={} db={} at-rest={}",
    ...
    let store = Store::open(&cfg.db_path, cfg.readers, key).map_err(|e| {
        let hint = if key.is_some() {
            " — wrong VESTA_SYNC_DB_KEY, or the DB exists and is not SQLCipher-encrypted?"
        } else {
            ""
        };
        format!("vesta-sync: cannot open DB at {}: {e}{hint}", cfg.db_path)
    })?;
    let bind = cfg.bind;
    ...
    let metrics_bind =
        std::env::var("VESTA_SYNC_METRICS_BIND").unwrap_or_else(|_| "127.0.0.1:8301".to_owned());
    if metrics_bind_allowed(&metrics_bind, "VESTA_SYNC_METRICS_ALLOW_PUBLIC") {
        let metrics_state = state.clone();
        tokio::spawn(async move {
            match tokio::net::TcpListener::bind(&metrics_bind).await {
                Ok(l) => {
                    eprintln!("vesta-sync: metrics on http://{metrics_bind}/metrics");
                    let _ = axum::serve(l, http::metrics_router(metrics_state)).await;
                }
                Err(e) => eprintln!("vesta-sync: metrics listener disabled ({e})"),
            }
        });
    } else {
        eprintln!(
            "vesta-sync: metrics listener DISABLED — {metrics_bind} is not loopback; /metrics \
             exposes op/device counts. Bind 127.0.0.1, or set VESTA_SYNC_METRICS_ALLOW_PUBLIC=1."
        );
    }

    let listener = tokio::net::TcpListener::bind(bind).await?;
    eprintln!("vesta-sync listening on {bind}");
...
async fn shutdown_signal() {
    ...
    eprintln!("vesta-sync: shutting down");
}
