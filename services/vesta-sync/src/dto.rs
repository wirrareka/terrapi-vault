//! Wire types for the vesta-sync oplog API (`spec/sync-openapi.yaml`). All payloads are
//! opaque to the server: `encrypted_payload` is client-side AEAD ciphertext and the server
//! never holds the vault key. See `docs/planning/02-vesta-sync-oplog.md`.
...
pub use vesta_transport::http::{Ack, ErrorBody};
