...
pub type EnrollRecord = (Vec<u8>, terrapi_vesta::KdfParams, Vec<u8>);
...
mod tests {
    ...
    fn file_store_with_reader_pool_reads_after_write() {
        ...
        let path = std::env::temp_dir().join(format!("vesta-sync-pool-{}.db", std::process::id()));
    ...
    fn encrypted_store_roundtrips_and_rejects_wrong_key() {
        ...
        let path = std::env::temp_dir().join(format!("vesta-sync-enc-{}.db", std::process::id()));
        let p = path.to_string_lossy().to_string();
        let clean = |p: &str| {
            for s in ["", "-wal", "-shm"] {
