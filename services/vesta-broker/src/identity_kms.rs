...
//! material (`VESTA_TLS_*`: cert+key as the client identity, the fleet Root CA bundle as the
//! trust root for identity's server cert). The whole path is **disabled unless
//! `VESTA_IDENTITY_KMS_URL` is configured** — until then the broker unseals with the manual
...
/// The inert sealed-master blob persisted at rest (`VESTA_SEALED_MASTER_FILE`). Opaque to the
