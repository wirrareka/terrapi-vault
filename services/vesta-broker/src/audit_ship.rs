...
//!
//! Enabled by `VESTA_AUDIT_OS_URL` (+ `_USER` / `_PASSWORD`, optional `_INSECURE_TLS=1`).
...
use vesta_transport::audit::{AuditSink, HashChainSink};
...
impl ShipConfig {
    fn from_env() -> Option<Self> {
        let base_url = std::env::var("VESTA_AUDIT_OS_URL").ok()?;
        let insecure = std::env::var("VESTA_AUDIT_OS_INSECURE_TLS").as_deref() == Ok("1");
        // Disabling audit-stream TLS verification exposes the OpenSearch credential (and the
        // audit feed) to a MITM, so — exactly like `VESTA_OS_INSECURE_TLS` in opensearch.rs — it
        ...
        if insecure && std::env::var("VESTA_ALLOW_INSECURE_DEV").as_deref() != Ok("1") {
            eprintln!(
                "vesta-broker: audit shipper DISABLED — VESTA_AUDIT_OS_INSECURE_TLS=1 disables \
                 TLS verification and is refused outside VESTA_ALLOW_INSECURE_DEV=1; point \
                 VESTA_AUDIT_OS_URL at a CA-trusted endpoint (or set VESTA_AUDIT_OS_CA)."
            );
            return None;
        }
        Some(Self {
            base_url: base_url.trim_end_matches('/').to_string(),
            user: std::env::var("VESTA_AUDIT_OS_USER").unwrap_or_else(|_| "admin".into()),
            password: std::env::var("VESTA_AUDIT_OS_PASSWORD").unwrap_or_default(),
            insecure,
            ca_path: std::env::var("VESTA_AUDIT_OS_CA")
                .ok()
                .filter(|s| !s.is_empty()),
        })
    }
...
pub fn build(cfg: &BrokerConfig) -> (Arc<dyn AuditSink>, Option<ShipTask>) {
    ...
    let client = match crate::opensearch::build_os_client(
        ...
        Ok(c) => c,
        Err(e) => {
            eprintln!("vesta-broker: audit shipper DISABLED (http client: {e})");
            return (sink, None);
        }
    };
    let mut cursor_path = cfg.audit_path.clone();
    cursor_path.set_extension("shipped");
    eprintln!("vesta-broker: audit shipping to OpenSearch enabled (tails the durable chain)");
    let task = ShipTask {
        client,
...
async fn ship_backlog(task: &ShipTask) {
    ...
    .await
    {
        ...
        Err(e) => eprintln!("vesta-broker: audit ship failed ({n} events): {e}"),
    }
}
...
mod tests {
    ...
    /// `VESTA_AUDIT_OS_TEST_URL` is set (docs/dev/opensearch-it.md for the docker cluster).
    #[tokio::test]
    async fn bulk_ships_events_into_an_index() {
        let Ok(url) = std::env::var("VESTA_AUDIT_OS_TEST_URL") else {
            eprintln!("skipping: VESTA_AUDIT_OS_TEST_URL unset");
            return;
        };
        let user = std::env::var("VESTA_AUDIT_OS_TEST_USER").unwrap_or_else(|_| "admin".into());
        let pass =
            std::env::var("VESTA_AUDIT_OS_TEST_PASSWORD").expect("VESTA_AUDIT_OS_TEST_PASSWORD");
