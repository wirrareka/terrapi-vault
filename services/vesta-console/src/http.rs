...
//! `VESTA_CONSOLE_ALLOW_INSECURE_DEV`. The SPA is served by the Vite dev proxy (dev) / embedded
...
async fn auth_callback(
    ...
    if let Some(err) = q.error {
        ...
        eprintln!("vesta-console: OIDC callback returned error: {err} {desc}");
        return (
    ...
    if cookie_value(&headers, AUTH_COOKIE_NAME).as_deref() != Some(state_binding(&state).as_str()) {
        eprintln!("vesta-console: OIDC callback state not bound to this browser (login-CSRF?)");
        return (
            AppendHeaders([(header::SET_COOKIE, cleared_auth_binding_cookie())]),
            (
    ...
    match oidc.complete(&code, &verifier, &nonce).await {
        ...
        Err(e) => {
            ...
            eprintln!("vesta-console: login failed: {e}");
