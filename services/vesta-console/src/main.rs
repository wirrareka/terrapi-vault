//! vesta-console — operator web/API console for terrapi-vault, one per residency group.
//! Read-only observability that aggregates the group's brokers' `observe` API over mTLS-over-WG.
//! NEVER surfaces a secret value. See `docs/planning/02-vesta-console.md`.
//!
...
async fn main() {
    ...
    eprintln!(
        "vesta-console: group={} brokers={} oidc={} dev={}",
        cfg.residency_group,
        cfg.brokers.len(),
        oidc.is_some(),
    ...
    let listener = tokio::net::TcpListener::bind(cfg.bind)
        .await
        .unwrap_or_else(|e| {
            eprintln!("vesta-console: bind {} failed: {e}", cfg.bind);
            std::process::exit(1);
        });
    eprintln!("vesta-console: listening on {}", cfg.bind);
    axum::serve(listener, http::router(state))
        .await
        .expect("server error");
}

/// Refuse the dangerous combinations of `VESTA_CONSOLE_ALLOW_INSECURE_DEV=1`. That single flag
...
///   * together with mTLS material (`VESTA_CONSOLE_TLS_*`) — that pairing is a production downgrade
...
fn check_dev_safety(cfg: &config::ConsoleConfig) {
    ...
    if !cfg.bind.ip().is_loopback() {
        eprintln!(
            "vesta-console: VESTA_CONSOLE_ALLOW_INSECURE_DEV=1 with a non-loopback bind ({}) is \
             ...
             VESTA_CONSOLE_TLS_* + OIDC for production.",
            cfg.bind
        );
        std::process::exit(1);
    }
    if cfg.tls.is_some() {
        eprintln!(
            "vesta-console: VESTA_CONSOLE_ALLOW_INSECURE_DEV=1 together with VESTA_CONSOLE_TLS_* \
    ...
    eprintln!(
        "vesta-console: WARNING VESTA_CONSOLE_ALLOW_INSECURE_DEV=1 — operator auth bypassed and \
...
async fn build_oidc(cfg: &config::ConsoleConfig) -> Option<Arc<oidc::OidcClient>> {
    let Some(oc) = cfg.oidc.clone() else {
        if !cfg.allow_insecure_dev {
            eprintln!(
                "vesta-console: WARNING no VESTA_CONSOLE_OIDC_ISSUER and not dev → all /api/v1 reads will 401"
            );
        }
        return None;
    };
    match oidc::OidcClient::build(oc).await {
        Ok(c) => Some(Arc::new(c)),
        Err(e) => {
            eprintln!("vesta-console: OIDC init failed: {e}");
            std::process::exit(1);
        }
    }
}
