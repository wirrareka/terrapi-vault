# terrapi-vault broker — FreeBSD deploy module
...
## Layout
- `build.sh` — build the `vault-broker` release binary (`cargo build --release -p vault-broker`).
- `jail/Bastillefile` + `jail/provision.sh` — stand up the vnet jail + delegate the
  encrypted dataset.
- `rc.d/vault-broker` — service (unprivileged `vault` user; `REQUIRE zfskeys`).
...
- `vault-broker.env.sample` — the env drop-in (`VAULT_*`, mode 0600, secrets from the dataset).
- `roles.json.sample` — `VAULT_ROLES_CONFIG`: SAN(dNSName) → {role, caps} (required in prod).
- `security/` — `pf.conf.snippet` (WG-only :8200/:8201), `fim-watchlist.txt`,
  `least-privilege.sh`, `audit_control.snippet`.
- `alerts/vault-broker-alerts.yml` — email-first Prometheus rules (`vault_sealed`, scrape-up,
...
2. **build:** `build.sh`; copy the binary to `deploy/jail/usr/local/sbin/vault-broker` before templating.
...
4. **jail:** `install.sh <group>` — unlock dataset → drop secrets (`unseal.pass`, `os-admin.pass`)
   → config (`vault-broker.env`, `roles.json`, `tls/`) → enable → start.
...
## Crown jewels (encrypted ZFS dataset `zroot/terrapi/vault` → `/var/db/terrapi-vault`)
