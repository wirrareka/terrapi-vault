...
memory `vault-sync-oplog-decision` and `docs/planning/02-vault-sync-oplog.md`).
...
device ed25519 pubkeys, and opaque encrypted ops; it never holds the vault key or plaintext.

- `store.rs` — SQLite (the lib's `rusqlite`; plain sqlite, payloads already E2E). Tables
  `accounts` / `devices` / `ops`; per-vault monotonic `seq` allocated in the push tx;
  idempotent dedupe by `(vault_id, op_id)`; per-vault isolation.
...
- `docs/planning/02-vault-sync-oplog.md` — design + status.
...
`02-vault-sync-oplog.md` for memento to implement. No `CONTRACTS.md` row (that file tracks the
